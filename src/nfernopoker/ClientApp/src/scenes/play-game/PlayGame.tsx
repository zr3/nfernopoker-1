...
class PlayGameComponent extends React.Component<IProps, ITempState> {
    ...
    getCurrentStory() {
        let story = this.props.game && this.props.game.stories.find(s => s.id == this.state.currentStoryId);
        return story ? story : null;
    }

    async onPointsAssigned(clearPoints?: boolean) {

        //this.props.firebase.watchEvent('child_changed', null, null, {
        //    path: `/games/${this.props.gameKey}`, storeAs: currentGame
        //});

        let currentStoryState: Story = { ...this.getCurrentStory() } as Story;
        currentStoryState.storyPoints = clearPoints ? "" : this.storyAverageScore(currentStoryState).toString();

        let stories = this.props.game.stories.map(s => s.id == currentStoryState.id ? currentStoryState : s);
    ...
    render() {
        ...
        let isInGame = game && game.team.players.some(p => p.email == this.props.profile.email);
        let currentStoryId = this.state.currentStoryId;
