  ...
  if (currentStory && !currentStory.id) {
    ...
    return (<React.Fragment>
      ...
      {currentStory.storyPoints &&
        <Typography color="primary" variant="subtitle2" align="left">Assigned Points: {currentStory.storyPoints}</Typography>
      }
      {isGameOwner &&
        <React.Fragment>
          <Typography color="secondary" variant="subtitle2" align="left">Average Score: {averageScore}</Typography>
          <Button variant="outlined" color="primary" size="small" onClick={() => assignStoryPoints()}>
            Assign Story Points
          </Button>
          <Button variant="outlined" color="primary" size="small" onClick={() => assignStoryPoints(true)}>
            Clear Points (Replay)
          </Button>
